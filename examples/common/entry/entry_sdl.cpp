...
#	endif // BX_PLATFORM_
	...
	struct Context
	{
		...
		bool init(int _argc, char** _argv, const void* _winid, uint32_t _w, uint32_t _h)
		{
			m_mte.m_argc = _argc;
			m_mte.m_argv = _argv;
		...
		bool do_step()
		{ 
			WindowHandle& defaultWindow = m_inner_handle;
			SDL_Event event;
			bool loop = true;
...
bool CocBGFXEntry::do_step()
{
	if (m_isinit)
	{
		return entry::s_ctx.do_step();
	}

	return false;
}

int CocBGFXEntry::finallize()
{
	if (m_isinit)
	{
		m_isinit = false;
